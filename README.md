...
We also try to introduce a learning rate decay per "epoch" by providing a callback function in kwarg of `model.fit`. The decay factor is set to 0.25 to observe what happens when learning decays too quickly, resulting in stagnant decrease in both training and validation loss (see output/cifar10_lr_decay_f0.25_plot.png)
