...
# if lb.classes_ == 2, e.g., labels = lb.fit_transform(["cat", "dog"]) =
# [[0]
#  [1]]
# the function to_categorical takes the above to [[1 0]
# 												  [0 1]]
# which will be then consistent with the ouput of lb.fit_transform
# when clases >= 3 (which returns a [None, 4] dimensional np array).
...
bbox_head = Dense(4, activation="sigmoid",
                  name="bounding_box")(bbox_head)
...
softmax_head = Dropout(0.5)(softmax_head)
softmax_head = Dense(len(lb.classes_), activation="softmax",
                     name="class_label")(softmax_head)
